...
class PromptCacheStrategy {
  ...
  buildCachedRequest({ systemPromptKey, documentText, userMessage }) {
    ...
    const req = {
      ...
      system: [
        { type: "text", text: pc.prompt, cache_control: { type: "ephemeral" } },
      ],
      messages: [],
    };
    if (documentText) {
      ...
      req.messages.push({ role: "user", content: userMessage });
    }
    return req;
  }

  buildCachedConversation({
    systemPromptKey,
    documentText,
    conversationHistory,
    newMessage,
  ...
  getStats() {
    ...
    return {
      ...
      hitRate: t > 0 ? ((this.cacheHits / t) * 100).toFixed(1) + "%" : "N/A",
    };
  }
}
