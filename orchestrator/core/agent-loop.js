class AgentLoop {
  ...
  async run({ systemPrompt, userMessage, context = {}, provider = "claude" }) {
    ...
    messages.push({
      role: "user",
      content: this.buildContextualMessage(userMessage, context),
    });
    ...
    while (iteration < this.maxIterations) {
      ...
      const response = await this.llmClient.call({
        provider,
        model: provider === "claude" ? "claude-sonnet-4-20250514" : undefined,
        messages,
        systemPrompt,
      ...
      if (response.tokensUsed) {
        totalTokensUsed.input += response.tokensUsed.input || 0;
        totalTokensUsed.output += response.tokensUsed.output || 0;
        totalTokensUsed.cacheRead += response.tokensUsed.cacheRead || 0;
        totalTokensUsed.cacheCreation += response.tokensUsed.cacheCreation || 0;
      }
      ...
      if (response.toolUse?.length > 0) {
        ...
        assistantContent.push(...response.toolUse);
        messages.push({ role: "assistant", content: assistantContent });
        const results = await Promise.all(
          response.toolUse.map(async (tc) => {
            try {
              return {
                type: "tool_result",
                tool_use_id: tc.id,
                content: JSON.stringify(
                  await this.toolRegistry.execute(tc.name, tc.input),
                ),
              };
            } catch (e) {
              return {
