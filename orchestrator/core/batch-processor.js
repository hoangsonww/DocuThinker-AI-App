class BatchProcessor {
  constructor({ llmClient, costTracker, logger } = {}) {
  ...
  async processBatch(documents, operation, options = {}) {
    const results = [],
      errors = [],
      batches = this.chunkArray(documents, this.BATCH_SIZE);
    for (let i = 0; i < batches.length; i += this.MAX_CONCURRENT) {
      const concurrent = batches.slice(i, i + this.MAX_CONCURRENT);
      const batchResults = await Promise.allSettled(
        concurrent.map((batch, idx) =>
          this.processSingleBatch(batch, operation, i + idx, options),
        ),
      );
    ...
    return {
      ...
      successRate:
        ((results.length / Math.max(documents.length, 1)) * 100).toFixed(1) +
        "%",
    };
  }
  ...
  async processSingleBatch(batch, operation, batchIndex, options) {
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      try {
        results.push({
          documentId: doc.id || doc._id || `doc-${batchIndex}-${i}`,
          status: "success",
          data: await this.executeOperation(doc, operation, options),
        });
      } catch (e) {
        results.push({
          documentId: doc.id || doc._id || `doc-${batchIndex}-${i}`,
