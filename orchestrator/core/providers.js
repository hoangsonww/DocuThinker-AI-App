class UnifiedLLMClient {
  constructor({ circuitBreaker, logger } = {}) {
    ...
    if (process.env.ANTHROPIC_API_KEY) {
      try {
        ...
        this.clients.claude = new A({ apiKey: process.env.ANTHROPIC_API_KEY });
      } catch (e) {
        this.logger.warn("[LLM] Anthropic SDK not available");
      }
    }
    if (process.env.GOOGLE_AI_API_KEY) {
      try {
        const { GoogleGenerativeAI } = require("@google/generative-ai");
        this.clients.gemini = new GoogleGenerativeAI(
          process.env.GOOGLE_AI_API_KEY,
        );
      } catch (e) {
        this.logger.warn("[LLM] Gemini SDK not available");
      }
    }
  }
  ...
  async callClaude({
    ...
    return {
      ...
      toolUse: r.content.filter((c) => c.type === "tool_use"),
    };
  }
  ...
  async callGemini({ model, messages, maxTokens, systemPrompt, temperature }) {
    ...
    const gm = this.clients.gemini.getGenerativeModel({
  ...
  async callWithFailover(options) {
    const order = ["claude", "gemini"].filter(
      (p) =>
        p !== options.provider &&
        this.circuitBreaker.canRequest(p) &&
        this.clients[p],
    );
    for (const fb of order) {
      try {
        ...
        if (result) {
          this.circuitBreaker.recordSuccess(fb);
          return result;
        }
      } catch (e) {
        this.circuitBreaker.recordFailure(fb);
        continue;
      }
    }
