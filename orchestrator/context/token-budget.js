class TokenBudgetManager {
  ...
  check({
    ...
    const cw = TokenBudgetManager.CONTEXT_WINDOWS[model] || 128000,
      avail = cw - reserveForOutput;
    let used = this.estimateTokens(
    ...
    return {
      ...
      overflow: used > avail ? used - avail : 0,
      recommendation:
        used > avail * 0.8 ? "Consider summarizing conversation history" : null,
    };
  }
  ...
  async compactContext({ messages, llmClient, keepLast = 4 }) {
    ...
    try {
      ...
      return [
        { role: "user", content: `[Previous conversation: ${r.content}]` },
        {
          role: "assistant",
          content: "I have context from earlier. Continue.",
        },
        ...toKeep,
      ];
    } catch (e) {
